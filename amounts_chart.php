...
$sql = "SELECT trxn_amount, count(id) as num_trxn, sum(amount) as amount_spent from (
(select a.*, 'Small' as trxn_amount from trxn a where amount <= 10)
union
(select a.*, 'Medium' as trxn_amount from trxn a where amount >10 and amount <50)
union
(select b.*, 'Large' as trxn_amount from trxn b where amount >=50)
) as trxn_amount group by 1 order by 1 desc";
...
$myPicture->drawText(250,55,"Spending by Amount Bucket",array("FontSize"=>12,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE));
$myPicture->drawScale();
$myPicture->drawBarChart(array("DisplayValues"=>TRUE,"DisplayColor"=>DISPLAY_AUTO,"Rounded"=>TRUE,"Surrounding"=>60));
//$myPicture->drawSplineChart();
$myPicture->autoOutput("amounts.png");
?>
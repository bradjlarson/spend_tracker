...
$sql = "SELECT case when num_trxn > 1 then spend_locale else 'One-off' end as locale_type, sum(num_trxn) as num_trxn, sum(trxn_amount) as trxn_amount from (
select spend_locale, count(id) as num_trxn, sum(amount) as trxn_amount from trxn where left(current_date,7) = left(date_time,7) group by 1
) as locales 
...
$myPicture->drawText(250,55,"Spending by Location",array("FontSize"=>12,"Align"=>TEXT_ALIGN_BOTTOMMIDDLE));
$myPicture->drawScale();
$myPicture->drawBarChart(array("DisplayValues"=>TRUE,"DisplayColor"=>DISPLAY_AUTO,"Rounded"=>TRUE,"Surrounding"=>60));
//$myPicture->drawSplineChart();
$myPicture->autoOutput("locale.png");
?>
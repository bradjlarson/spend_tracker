...
$sql = "SELECT CASE when joint = 1 then 'Sarah and I' when joint = 2 then 'Kastle' else 'Other' end as grouping, count(id) as num_trxn, sum(amount) as trxn_amount from trxn where left(current_date,7) = left(date_time,7) group by 1";
$Result  = mysql_query($sql);
$month_day=""; $amount_spent=""; $num_trxn="";
while($row = mysql_fetch_array($Result))
 {
  /* Push the results of the query in an array */
  $month_day[]   = $row["grouping"];
...
$myPicture->drawText(250,55,"Spending by Group",array("FontSize"=>12,"Align"=>TEXT_ALIGN_TOPMIDDLE));
$myPicture->drawScale();
$myPicture->drawBarChart(array("DisplayValues"=>TRUE,"DisplayColor"=>DISPLAY_AUTO,"Rounded"=>TRUE,"Surrounding"=>60));
//$myPicture->drawSplineChart();
$myPicture->autoOutput("joint.png");
?>